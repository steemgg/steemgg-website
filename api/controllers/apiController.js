...
exports.updateGame = async function(req, res, next) {
    try{
        ...
        if(typeof data.recommend !== 'undefined') {
            ...
            dbRes = await game.updateGame([{ recommend:data.recommend,lastModified:unix }, req.params.id, req.session.user.userid]);
        } else {
            dbRes = await game.updateGameSelf([{ title:data.title,coverImage:data.coverImage,description:data.description,category:data.category,gameUrl:data.gameUrl,lastModified:unix,width:data.width,height:data.height }, req.params.id, req.session.user.userid]);
        }
