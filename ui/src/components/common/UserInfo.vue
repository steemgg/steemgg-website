  ...
  export default {
    ...
    methods: {
      ...
      login () {
        ...
        window.location.href = 'https://v2.steemconnect.com/oauth2/authorize?client_id=steemgg.app&redirect_uri=' + encodeURIComponent(process.env.API_SERVER_URL + 'callback') + '&scope=login,vote,comment,delete_comment,comment_options,custom_json,claim_reward_balance&response_type=code&state=' + window.location.href
      }
    },
    mounted () {
      if (this.testMode) {
